...
#metabolite <- xml2::as_list(metabolites[[1]])
#names(metabolite)
num <- length(metabolites)
#load("./metabolites_tibble.RData")
metabolites_tibble[1, ]$ontology
...
process_ontology_root(root = metabolite$ontology[[1]])

metabolitesList <- lapply(1:num, function(i) {
  print(paste0(i, " / ", num))
  xml2::as_list(metabolites[[i]])
})
#save(metabolitesList, file = "metaboliteList.RData")

# 105985, a blank example.
...
#saveRDS(hmdbCmpTb, file = "hmdbCmpTb.rds")
